...
@Component("T_Data_AuthDao")
public class T_Data_AuthDao extends ServerBase implements IDao {
	private static List<String> clmList = new ArrayList<String>();
	private static List<String> baseList = new ArrayList<String>();
	static {
		clmList.add("object_no");
		clmList.add("object_type");
		clmList.add("data_auth_no");
		clmList.add("data_auth_type");
		clmList.add("data_auth_child");
		clmList.add("is_half_check");
		clmList.add("is_display");
	...
	public int insert(Map<String, Object> param) {
		...
		String value = "?, ?, ?, ?, ?, ?";
		update.add(UniqueSeq.getUniqueSeq("serno", "t_data_auth"));
		...
		String sql = "INSERT INTO t_data_auth (" + clm + ") VALUES(" + value + ")";
		update.setSql(sql);
	...
	public int update(Map<String, Object> where, Map<String, Object> set) {
		...
		String sql = "UPDATE t_data_auth SET " + setClm + " " + whereClm;
		update.setSql(sql);
		return this.dbHandle().update(update);
	}
	...
	public int delete(Map<String, Object> where) {
		...
		whereClm += " AND data_auth_no not in (select branch_id from sys_branch where status = ? )";
		update.add("0");
		String sql = "DELETE FROM t_data_auth " + whereClm;
		update.setSql(sql);
		return this.dbHandle().update(update);
	}
	...
	private boolean isContinue(String clmName) {
		if (T_Data_AuthDao.clmList.contains(clmName)) {
			return true;
		}
		if (T_Data_AuthDao.baseList.contains(clmName)) {
	...
	private QuerySqlable joinQuerySql(Map<String, Object> param,
			boolean isPaginate) {
		String sql = "SELECT * FROM t_data_auth ";
		String where = "";
