...
@Component("T_Data_Auth_TypeDao")
public class T_Data_Auth_TypeDao extends ServerBase implements IDao {
	...
	static {
		...
		clmList.add("data_auth_type_name");
		clmList.add("data_auth_source");
		clmList.add("mapping_type");
		clmList.add("mapping_tab");
		clmList.add("mapping_no");
		clmList.add("mapping_name");
		clmList.add("mapping_parent");
		clmList.add("mapping_rule");
		baseList.add("serno");
	...
	public int insert(Map<String, Object> param) {
		...
		String value = "?, ?, ?, ?, ?, ?";
		update.add(UniqueSeq.getUniqueSeq("serno", "t_data_auth_type"));
		...
		String sql = "INSERT INTO t_data_auth_type (" + clm + ") VALUES(" + value + ")";
		update.setSql(sql);
	...
	public int update(Map<String, Object> where, Map<String, Object> set) {
		...
		String sql = "UPDATE t_data_auth_type SET " + setClm + " " + whereClm;
		update.setSql(sql);
		return this.dbHandle().update(update);
	}
	...
	public int delete(Map<String, Object> where) {
		...
		String sql = "DELETE FROM t_data_auth_type " + whereClm;
		update.setSql(sql);
		return this.dbHandle().update(update);
	}
	...
	private boolean isContinue(String clmName) {
		if (T_Data_Auth_TypeDao.clmList.contains(clmName)) {
			return true;
		}
		if (T_Data_Auth_TypeDao.baseList.contains(clmName)) {
	...
	private QuerySqlable joinQuerySql(Map<String, Object> param,
			boolean isPaginate) {
		String sql = "SELECT * FROM t_data_auth_type ";
		String where = "";
