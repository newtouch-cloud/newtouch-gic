...
@Controller
public class CmainPolicyImportController extends BaseController{
	@Autowired private ICmainPolicyService cmainPolicyService ;
	/**
	 * 转到上传页面
	 */
	@RequestMapping("/CBS/policyImport/policyUpload.do")
	public String toPolicyUpload(){
		return "ca/cacore/cbs/policyLife/propertyInsuranceUpLoad";
	}
	...
	* @description:
	 */
	 @RequestMapping("/CBS/policyImport/uploadImportInsurance.do")
	 public void importCarInsurance(HttpServletRequest req, HttpServletResponse resp) throws Exception{
		...
		* @description: 保单导入
		 */
	    @RequestMapping("/CBS/policyImport/importInsurance.do")
		...
		* @description: 保单导入
		*/
	    @RequestMapping("/CBS/policyImport/importInsurance2.do")
	    public void importPolicy2(HttpServletRequest req, HttpServletResponse resp) throws IOException{
			...
			try {
				for (MultipartFile file : files.values()) {
					System.out.println("导入文件名称为" + file.getOriginalFilename());
		...
		* @description: 保单导入
		 */
	    @RequestMapping("/CBS/policyImport/importInsurancePerson.do")
	  ...
	  /**
		...
		* @description: 保单导入
		 */
	    @RequestMapping("/CBS/policyImport/importBusiness.do")
	    public void importBusiness(HttpServletRequest req, HttpServletResponse resp) throws IOException{
			...
			try {
				for (MultipartFile file : files.values()) {
					System.out.println(file.getOriginalFilename());
					info = cmainPolicyService.importBusiness(file, user);
				}
				//resp.getWriter().write("{\"uploadMsg\": \"failure\"}");
				resp.getWriter().write("{\"ret\": \"0000\",\"uploadMsg\": \""+info+"\"}");
			} catch (Exception e) {
		...
		* @description: 保单导入
		 */
	    @RequestMapping("/CBS/policyImport/importInter.do")
	    public void importInter(HttpServletRequest req, HttpServletResponse resp) throws IOException{
			...
			try {
				for (MultipartFile file : files.values()) {
					System.out.println(file.getOriginalFilename());
					info = cmainPolicyService.importInter(file, user);
				}
				//resp.getWriter().write("{\"uploadMsg\": \"failure\"}");
				resp.getWriter().write("{\"ret\": \"0000\",\"uploadMsg\": \""+info+"\"}");
			} catch (Exception e) {
