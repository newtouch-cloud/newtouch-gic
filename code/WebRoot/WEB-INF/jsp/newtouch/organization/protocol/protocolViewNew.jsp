  ...
  $(document).ready(function() {
	...
	<body style="height: 750px">
		<div class="container-fluid" >
			...
			<!--增加面板 start -->
			<div class="row-fluid">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/organization/Protocol/modifyProtocol.do" method="POST" enctype="multipart/form-data">
					<!-- 提示信息标签 -->
					...
					<div class="row" style="margin-left:-30px;margin-right:-40px">
    					 ...
    					 <div class="control-group span4">    
							...
							<select  id="protocol_category" name="protocol_category" class="input-medium null" readonly="true"></select>
							</div></div>
    					<div class="control-group span4">    
							<label class="control-label" for="contract_type">合同类型</label> <div class="controls"> 
							<select  id="contract_type" name="contract_type" class="input-medium null" readonly="true"></select>
							</div>
						</div> 
					...
					<div class="row" style="margin-left:-30px;margin-right:-40px">
				  		<webTag:Text id="party_a"  name="party_a"  value='${rmHelper.returnParams.party_a}' displayLable="甲方" readonly="true"/>
 						<webTag:Text id="party_b"  name="party_b"  value='${rmHelper.returnParams.party_b}' displayLable="乙方" readonly="true"/>
						<webTag:Date id="dateofsign"  name="dateofsign" value='${rmHelper.returnParams.dateofsign}'   displayLable="协议签订日期" dateFormat="yyyy-MM-dd" isdisplay="true" readonly="true"/>           
					</div>
					<div class="row" style="margin-left:-30px;margin-right:-40px">     
						<webTag:Date id="startdate"   name="startdate"  value='${rmHelper.returnParams.startdate}'    displayLable="协议生效起期"    dateFormat="yyyy-MM-dd"   isdisplay="true" readonly="true"/>
						<webTag:Date id="enddate"     name="enddate"    value='${rmHelper.returnParams.enddate}'      displayLable="协议生效止期"    dateFormat="yyyy-MM-dd"   isdisplay="true" readonly="true"/> 
						 <div class="control-group span4">    
							<label class="control-label" for="sign_type">签订类型</label> <div class="controls"> 
							<select  id="sign_type" name="sign_type" class="input-medium null" isdisplay="true" readonly="true"></select>
					...
					<div class="control-group span4">    
							...
							<select  id="status" name="status" class="input-medium null" isdisplay="true" readonly="true"></select>
							</div></div>
    ...
    function objdisabled(){
		var myselect=document.getElementById("sales_status"); 
		var index=myselect.selectedIndex ;             // selectedIndex代表的是所选中项的index
		var se_value = myselect.options[index].value;
		if(se_value=="1"){
			document.getElementById("shopowner_value").disabled=true;
			document.getElementById("staff_value").disabled=true;
		}else{
			document.getElementById("shopowner_value").disabled=false;
			document.getElementById("staff_value").disabled=false;
		}
	}
    
    $(function(){ 
    	 $("#protocol_category").change();
    })
</script>
</html>
