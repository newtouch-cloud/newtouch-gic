...
<script type="text/javascript">
		//验证协议为保险公司协议时 保险公司必选，为代理时 维修码必填
		
		
		jQuery.validator.addMethod("checkData1",function(value,element){
			 var selectPositionCode=$("#protocol_category").val();
			 var ins_branchname = $("#ins_branchname").val();
			 if("1"== selectPositionCode && ins_branchname!=null && ins_branchname!=undefined && ins_branchname!=""){
				 return true;
			 }else if("1"== selectPositionCode && (ins_branchname==null || ins_branchname==undefined || ins_branchname=="")){
				 return false;
			 }else {
				 return true;
			 }
	...
	<div class="container-fluid">
		...
		<div class="row-fluid">
			<form id="queryForm" name="queryForm"
				...
				action="<%=basePath%>/organization/Protocol/modifyProtocol.do"
				...
				<webTag:HiddenInputTag name="agreement_no" id="agreement_no"
					value="${rmHelper.returnParams.agreement_no}"></webTag:HiddenInputTag>
				...
				<fieldset>
					<div class="row" style="margin-left: -30px; margin-right: -40px">
						...
						<div class="control-group span4">
							...
							<div class="controls">
								<select id="protocol_category" name="protocol_category"
									class="input-medium null"></select>
							</div>
						</div>
						<div class="control-group span4">
					...
					<div class="row" style="margin-left: -30px; margin-right: -40px">
						<webTag:Text id="party_a" name="party_a"
							value='${rmHelper.returnParams.party_a}' displayLable="甲方" />
						<webTag:Text id="party_b" name="party_b"
							value='${rmHelper.returnParams.party_b}' displayLable="乙方" />
						<webTag:Date id="dateofsign" name="dateofsign"
							value='${rmHelper.returnParams.dateofsign}' displayLable="协议签订日期"
							dateFormat="yyyy-MM-dd" isdisplay="true" />
					</div>
					<div class="row" style="margin-left: -30px; margin-right: -40px">
						<webTag:Date id="startdate" name="startdate"
							value='${rmHelper.returnParams.startdate}' displayLable="协议生效起期"
							dateFormat="yyyy-MM-dd" isdisplay="true" />
						<webTag:Date id="enddate" name="enddate"
							value='${rmHelper.returnParams.enddate}' displayLable="协议生效止期"
							dateFormat="yyyy-MM-dd" isdisplay="true" />
						<!--  <div class="control-group span4">
							<label class="control-label" for="sign_type">签订类型</label>
							<div class="controls">
				...
				<!-- 	<div class="row" style="margin-left: -30px; margin-right: -40px">				
					...
					<div class="row" style="margin-left: -30px; margin-right: -40px">
						<jsp:include page="/WEB-INF/jsp/ca/cacore/util/protocolTree.jsp"
							flush="true" />
						<webTag:Text id="push_code" name="push_code"
							value='${rmHelper.returnParams.push_code}' displayLable="代理推荐维修码" />
						<webTag:Text id="credit_code" name="credit_code"
							value='${rmHelper.returnParams.credit_code}'
							displayLable="代理社会统一信用码" />
					</div>
					...
					<div class="row" style="margin-left: -30px; margin-right: -40px">
						...
						<webTag:Text id="bank_name" name="bank_name"
							value='${rmHelper.returnParams.bank_name}' displayLable="代理开户行" />
						<webTag:Text id="bank_code" name="bank_code"
							value='${rmHelper.returnParams.bank_code}' displayLable="代理银行账号" />
					</div>
					<!-- 
					<div class="row" style="margin-left: -30px; margin-right: -40px">
						<webTag:Text id="sup_protocol_no" name="sup_protocol_no"
							value='${rmHelper.returnParams.sup_protocol_no}' displayLable="补充协议编号" />
						<webTag:Text id="sup_protocol_content" name="sup_protocol_content"
							value='${rmHelper.returnParams.sup_protocol_content}' displayLable="补充协议内容" />
						<webTag:Date id="sup_protocol_stadate" name="sup_protocol_stadate"
							value='${rmHelper.returnParams.sup_protocol_stadate}' displayLable="补充协议生效日期"
...
<script type="text/javascript">
    $(document).ready(function(){
    	//加载合同类型，协议类型
    	 $.ajax({
             type: "POST",
             url: "/CACore/organization/Protocol/proCategory.do",
             async : true,
             dataType: "json",
             success: function (respData) {
                 proStatusList = respData.retList;
                 var arr = [];
                 arr.push('<option value="">---请选择---</option>')
                 if(respData.retList){
                     for (var i = 0; i < proStatusList.length; i++) {
                         var item = proStatusList[i];
                         if(${rmHelper.returnParams.protocol_category}==item.code){
    ...
    $("#protocol_category").change(function () {
    	if($(this).val() != null && $(this).val() != "" && $(this).val() != "undefined" 
    		...
    		var proCategory=$(this).val();
    	}else {
    		var proCategory=${rmHelper.returnParams.protocol_category};
    	}
    $.ajax({
         type: "POST",
         url: "/CACore/organization/Protocol/proCategoryType.do",
         data:{"proCategory":proCategory},
         async : true,
         dataType: "json",
         success: function (respData) {
             proStatusList = respData.retList;
             ...
             if(respData.retList){
                 for (var i = 0; i < proStatusList.length; i++) {
                     var item = proStatusList[i];
                     if(${rmHelper.returnParams.contract_type}==item.code){
                         arr.push('<option value="' + item.code + '" selected>' + item.name + '</option>');
                       }else{
                         arr.push('<option value="' + item.code + '">' + item.name + '</option>');
                       }
    ...
    //	 
    	 ...
    	 $.ajax({
             ...
             success: function (respData) {
                 ...
                 if(respData.retList){
                     for (var i = 0; i < proStatusList.length; i++) {
                         var item = proStatusList[i];
                         if(${rmHelper.returnParams.status}==item.code){
                        	 arr.push('<option value="' + item.code + '" selected>' + item.name + '</option>');
                         }else{
                         arr.push('<option value="' + item.code + '">' + item.name + '</option>');
                    	 }
    	 ...
    	 $("#protocol_category").trigger("change");
    });
    ...
    $("#protocol_category").change(function (){
    	if($(this).val() != null && $(this).val() != "" && $(this).val() != "undefined" 
			...
			var selectPositionCode=${rmHelper.returnParams.protocol_category};
		}
