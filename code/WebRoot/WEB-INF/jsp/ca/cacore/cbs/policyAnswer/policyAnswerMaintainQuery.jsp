...
<html lang="en">
	<head>
		...
		<script >
			var linkUrl = "<%=basePath %>/policyAnswer/queryPolicyAnswerInfo.do";
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i>保单回访信息维护</span>
			...
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/policyAnswer/queryPolicyAnswerInfo.do" method="POST">
					  <div class="row">
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="cm" items="${rmHelper.returnMsg.dataList}" varStatus="index" >
							<tr>
							    <td>
							      <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/policyAnswer/goPolicyAnswerModify.do?seq_id=${cm.seq_id}'><i class="icon-pencil icon-white"></i>修改</a>
							   </td>
							    <td>
							    <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/policyAnswer/policyAnswerView.do?seq_id=${cm.seq_id}'><i class="icon-zoom-in icon-white"></i>明细</a>
							    </td>
							    <td>${index.index+1}</td>
								<td>${cm.branch_id}</td>
								...
								<td>${cm.answer_type}</td>
								<td>${cm.is_answered}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
