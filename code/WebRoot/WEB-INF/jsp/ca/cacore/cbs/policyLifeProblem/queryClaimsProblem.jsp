...
<html lang="en">
	<head>
		...
		<script type="text/javascript">
		    var linkUrl = "<%=basePath %>/CBS/PolicyLifeProblem/queryClaimsProblem.do";  
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i>理赔问题件管理</span>
			    <div class="slideUp_Down" id="Shrinkbutton1"></div>
			</div>
			<!-- 面包屑导航  end -->
			<!-- 查询面板 start -->
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/CBS/PolicyLifeProblem/queryClaimsProblem.do" method="POST">
					 <div class="row">
				    ...
				    <div class="row" style="text-align:right;">
				      	<button type="submit" class="btn btn-danger" onClick="defaultQuery();" ><i class="icon-search icon-white"></i>查询</button>
				   	    <a  class="btn btn-danger" href="<%=basePath %>/redirect/redirect.do?linkUrl=ca/cacore/cbs/policyLifeProblem/addClaimsProblem&flag=3" ><i class="icon-plus icon-white"></i>添加</a>
					</div>
				</form>
			</div>
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="sm" items="${rmHelper.returnMsg.dataList}" varStatus="index">
							<tr>
								<td>
								   <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/CBS/PolicyLifeProblem/getClaimsConserProblemView.do?seq_id=${sm.seq_id}&flag=4'><i class="icon-pencil icon-white"></i>状态更新</a>
							    </td>
							    <td>
								   <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/CBS/PolicyLifeProblem/getClaimsConserProblemView.do?seq_id=${sm.seq_id}&flag=2'><i class="icon-zoom-in icon-white"></i>明细</a>
							    </td>
								<td>${index.index+1}</td>
								<td>${sm.branch_id}</td>
