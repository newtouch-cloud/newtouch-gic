			 ...
			 jQuery.validator.addMethod("queryBranch", function(value, element) {
					...
					$("label[for='insBranch_id'][class='error']").remove();
			     	...
			     	if (!isUndefined(insBranch_id) && !isUndefined(policy_code)) {
			     		$.ajax({
			     			url : "<%=basePath %>/CBS/PolicyLifeProblem/getInfoForClaimsConser.do",
			     			type : "post",
			     			dataType : "json", 
			     			async : false,
			     			data : {
	...
	<body>
		<div class="container-fluid">
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i>保全问题件录入</span>
			</div>
			<!-- 面包屑导航  end -->
			
			<!-- 数据区 start -->
			<div class="row-fluid">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12"  action="<%=basePath %>/PolicyLifeProblem/addPolicyLifeProblem.do?flag=2" method="POST">
					...
					<webTag:HiddenInputTag id="send_code" name="send_code" value='${rmHelper.returnParams.send_code}' displayLable="所属投保单号" />
					...
					<div class="row">
						...
						<webTag:Text id="policy_code" name="policy_code" value='' displayLable="所属保单号"  isdisplay="true"/>
					</div>
					...
					<div class="row">
						...
						<webTag:DynamicSelectTag src="problemOriginTypeSelect" name="origin_type" id="origin_type" value='2' displayLable="问题件业务来源" isdisplay="true" onmousemove="this.setCapture();" onmouseout="this.releaseCapture();" onfocus="this.blur();" readonly="readonly"/>
						<webTag:DynamicSelectTag src="problemStatusSelect" name="status" id="status" value='${rmHelper.returnParams.status}'  displayLable="问题件状态" isdisplay="true"/>
					</div>
				    ...
				    <div class="row" style="text-align:right;">
  				         ...
  				         <a class="btn btn-danger" href='<%=basePath %>/CBS/PolicyLifeProblem/queryConservationProblem.do'><i class="icon-share-alt icon-white"></i>返回</a>
					</div>
				</form>
			</div>
			<!--  数据区 end -->
