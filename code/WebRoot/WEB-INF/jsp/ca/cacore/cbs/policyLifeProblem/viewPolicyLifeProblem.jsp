	...
	<body>
		<div class="container-fluid">
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i>投保单问题件明细</span>
			</div>
			<!-- 面包屑导航  end -->
				...
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/PolicyLifeProblem/modifyPolicyLifeProblem.do" method="POST">
				   ...
				   <div id="uploadimage" style="width:100%;">
									...
									<c:choose>
										<c:when test="${rmHelper.returnMsg.dataListSize == 0}">
											<div style='margin-left: 12px'>
												<webTag:ReturnMsgTag id="msg" name="msg" msg='没有相关联的影像信息!' />
											</div>
										</c:when>
										<c:when test="${rmHelper.returnMsg.dataList != null}">
											<c:forEach var="sm"
												items="${rmHelper.returnMsg.dataList}"
												varStatus="number">
												<li class='thumbnail'><img id="img1"
													src="<%=basePath %>/${sm.file_path}" class="magnify"
				    ...
				    <div class="row" style="text-align:right;">
					   <a id="viewBack" class="btn btn-danger" href="<%=basePath %>/CBS/PolicyLifeProblem/queryPolicyLifeProblem.do"><i class="icon-share-alt icon-white"></i>  返回</a>
					</div>
				</form>
			</div>
			<!--  数据区 end -->
