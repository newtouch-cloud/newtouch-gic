...
<html lang="en">
	<head>
		...
		<script >
			var linkUrl = "<%=basePath %>/cbs/policyLifeManage/queryPolicyLifeForStatus.do";
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i>投保单状态变更</span>
			...
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/cbs/policyLifeManage/queryPolicyLifeForStatus.do" method="POST">
				    <div class="row">
					...
					<div class="row">
					 <webTag:Text id="agent_name" name="agent_name" value='${rmHelper.returnParams.agent_name}' displayLable="中介人员姓名"/>
					    <webTag:DynamicSelectTag src="policyStatusSelect" name="status" id="status" parameter="6:200" value='${rmHelper.returnParams.status}' displayLable="投保单状态"/>
					</div>
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="cm" items="${rmHelper.returnMsg.dataList}" varStatus="index" >
							<tr>
							     <td>
							   		  <a id="changeStatus" class="btn btn-mini btn-dangerLight" href='<%=basePath %>/cbs/policyLifeManage/viewPolicyLife.do?seq_id=${cm.seq_id}&flag=3'><i class="icon-pencil icon-white"></i> 状态变更</a>
							     </td>
							    <td>${index.index+1}</td>
								<td>${cm.branch_id}</td>
