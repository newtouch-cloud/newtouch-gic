...
<html lang="en">
	<head>
		...
		<script >
			var linkUrl = "<%=basePath %>/CBS/Claims/queryClaimsList.do";
		...
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			   <span class=mrl14><i class="icon-list icon-red"></i> 保单管理</span><span class="divider">/</span>
			    ...
			    <span><i class="icon-list icon-red"></i> 理赔维护</span>
			    <div class="slideUp_Down" id="Shrinkbutton1"></div>
			</div>
			<!-- 面包屑导航  end -->
			<!-- 查询面板 start -->
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/CBS/Claims/queryClaimsList.do" method="POST">
				    <div class="row">
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="cm" items="${rmHelper.returnMsg.dataList}" varStatus="index">
							<tr>
							    <td>
							    	<a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/CBS/Claims/goModifyClaimsPage.do?seq_id=${cm.seq_id}'><i class="icon-pencil icon-white"></i>修改</a><!-- Edit -->
									<a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/CBS/Claims/viewClaims.do?seq_id=${cm.seq_id}'><i class="icon-zoom-in icon-white"></i>明细</a><!-- View -->
							    </td>
							    <td>${index.index+1}</td>
