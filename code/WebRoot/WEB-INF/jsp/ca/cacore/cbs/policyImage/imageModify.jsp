...
%>
  	 ...
  	 function deleteImage(file_id){
			...
			if(boo){
				...
				ei.hide(); //隐藏预览界面
				$('#flag').val("进行了修改");//表示进行了修改操作
				$('form').append("<input  type='hidden' name='delete_id'  value='"+file_id+"' />");
			}
		}
						   ...
						   <c:when test="${rmHelper.returnMsg.dataList != null}">
						   		<c:forEach var="sm" items="${rmHelper.returnMsg.dataList}" varStatus="number">
						    	    <li class='thumbnail'>	
						    	    	...
						    	    	<div id="${sm.file_id}" class="well gallery-controls" style="margin-top:-1px;width:38px;margin-left:73px;display:none;"><p><a class="btn" title="删除此影像" href="javascript:void(0)" onclick="deleteImage('${sm.file_id}');" ><i class="icon-remove"></i></a></p></div>
						    	    </li>
						    	</c:forEach>
						   </c:when>
						</c:choose>
