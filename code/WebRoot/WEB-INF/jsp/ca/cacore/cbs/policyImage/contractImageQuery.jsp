...
<html lang="en">
	<head>
		...
		<script >
			var linkUrl = "<%=basePath %>/cbs/policyImage/queryContractImage.do";
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i> 保单影像件查询</span>
			    <div class="slideUp_Down" id="Shrinkbutton1"></div>
			</div>
			<!-- 面包屑导航  end -->
			<!-- 查询面板 start -->
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/cbs/policyImage/queryContractImage.do" method="POST">
					<div class="row"> <!-- 机构选择 -->
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="cm" items="${rmHelper.returnMsg.dataList}" varStatus="index" >
							<tr>
							    <td>
						    		<a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/cbs/policyImage/toModifyContractImage.do?policy_id=${cm.policy_id}'><i class="icon-pencil icon-white"></i>修改</a>
						    		<a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/cbs/policyImage/viewContractImage.do?policy_id=${cm.policy_id}'><i class="icon-zoom-in icon-white"></i>明细</a>
							    </td>
							    <td>${index.index+1}</td>
								<td>${cm.branch_id}</td>
