...
<html lang="en">
	<head>
		...
		<script >
			var linkUrl = "<%=basePath %>/cbs/policyImage/queryPolicyImage.do";
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i> 投保单影像件查询</span>
			...
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/cbs/policyImage/queryPolicyImage.do" method="POST">
				    ...
				    <div class="row">
						...
						<webTag:Text id="policy_code" name="policy_code" value='${rmHelper.returnParams.policy_code}' displayLable="投保单号"/>
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="cm" items="${rmHelper.returnMsg.dataList}" varStatus="index" >
							<tr>
							    <td>
						    		<a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/cbs/policyImage/toModifyPolicyImage.do?policy_id=${cm.policy_id}'><i class="icon-pencil icon-white"></i>修改</a>
						    		<a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/cbs/policyImage/viewPolicyImage.do?policy_id=${cm.policy_id}'><i class="icon-zoom-in icon-white"></i>明细</a>
							    </td>
							    <td>${index.index+1}</td>
								<td>${cm.branch_id}</td>
