...
<html lang="en">
	<head>
		...
		<script >
			var linkUrl = "<%=basePath %>/cbs/policyLifeAck/queryPolicyLifeAckList.do";
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i>保单回执信息维护</span>
			...
			<div class="row-fluid" id="Shrinkcontent1">
					<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/cbs/policyLifeAck/queryPolicyLifeAckList.do" method="POST">
				    <div class="row">
			...
			<div class="overAuto row-fluid">
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
							...
							<tr>
							    <td>
							    <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/policyAnswer/goPolicyLifeAckModify.do?seq_id=${cm.seq_id}'><i class="icon-pencil icon-white"></i>修改</a>
							    </td>
							    <td>
							    <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/cbs/policyLifeAck/policyLifeAckView.do?seq_id=${cm.seq_id}'><i class="icon-zoom-in icon-white"></i>明细</a>
							    </td>
							    <td>${index.index+1}</td>
								<td>${cm.branch_id}</td>
								...
								<td>${cm.hold_date}</td>
								<td>${cm.ack_customer_date}</td>
								<td>${cm.ack_branch_date}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
