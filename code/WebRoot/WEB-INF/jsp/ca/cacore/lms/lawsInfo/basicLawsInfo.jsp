...
<html lang="en">
	<head>
		...
		<script >
			...
			//分页
			var linkUrl = "<%=basePath %>/LMS/BasicLawsManager/getAllBasicLawsInfo.do";
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i> 基本法信息维护</span>
			...
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/LMS/BasicLawsManager/getAllBasicLawsInfo.do" method="POST">
...
<%-- 						<image id="dialog_img" >${rmHelper1.msgStr}</image> --%>
					...
					<div class="row">
						<webTag:DynamicSelectTag src="channelInfoSelect" name="dept_type" id="dept_type" displayLable="渠道类型"></webTag:DynamicSelectTag>
				    ...
				    <div class="row" style="text-align:right;">
				    	...
				    	<a class="btn btn-danger"href="<%=basePath%>/redirect/redirect.do?linkUrl=ca/cacore/lms/lawsInfo/basicLawsAdd"><i class="icon-plus icon-white"></i>新增</a>
					</div>
				</form>
			</div>
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable ">
					...
					<tbody>
						<c:forEach var="sm" items="${rmHelper.returnMsg.dataList}" varStatus="index">
							<tr>
								<td class="FixedTd">
									<c:if test='${sm.data_flag == "N"}'>
										<a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/LMS/BasicLawsManager/getBasiclawView.do?serno=${sm.serno}'><i class="icon-pencil icon-white"></i>修改</a><!-- Edit -->
									</c:if>
									<a id="other" class="btn btn-mini btn-dangerLight" onclick="return delBasicLaws()" href='<%=basePath%>/LMS/BasicLawsManager/delBasicLaws.do?serno=${sm.serno}'><i class="icon-off icon-white"></i>删除</a>
								</td>
