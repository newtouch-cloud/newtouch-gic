...
<html lang="en">
	<head>
        ...
        <script >    
			var linkUrl = "<%=basePath %>/AMS/Announcement/queryAnnouncementAppr.do";
			...
			function importInfo(){
	               $('#queryForm').attr("action","<%=basePath %>/AMS/Announcement/exportAnnouncement.do"); //设置action指向导出
	               $('#queryForm').submit();
	               $('#queryForm').attr("action","<%=basePath %>/AMS/Announcement/queryAnnouncementAppr.do?pageName=announcementAppr_Query"); //设置action执向查询
	            }
	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i>公告审批查询</span>
			...
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/AMS/Announcement/queryAnnouncementAppr.do?pageName=announcementAppr_Query" method="POST">
					<!-- 提示信息标签 -->
					...
					<div class="row">
                         ...
                         <webTag:DynamicSelectTag src="approvalStatusSelect" name="approval_status" id="approval_status" displayLable="审批状态" value='${rmHelper.returnParams.approval_status}' />
					</div>
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="rb" items="${rmHelper.returnMsg.dataList}" varStatus="index" >
							<tr>
								<td>
								    <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/AMS/Announcement/viewAnnouncementAppr.do?approval_id=${rb.approval_id}'><i class="icon-zoom-in icon-white"></i>明细</a>
								</td>
							    <td>${index.index+1}</td>
								<td>${rb.branch_id}</td>
								...
								<td>${rb.application_time}</td>
								<td>${rb.approval_time}</td>
