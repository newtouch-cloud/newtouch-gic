	...
	<body>
		<div class="container-fluid" >
			...
			<div class="dreadcount">
			    ...
			    <span><i class="icon-list icon-red"></i> 公告维护</span>
			...
			<div class="row-fluid" id="Shrinkcontent1">
				<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath %>/AMS/Announcement/queryAnnouncement.do?pageName=announcement_Query" method="POST">
					<!-- 提示信息标签 -->
...
<%-- 						<webTag:DynamicSelectTag src="announcementTypeSelect" name="announcement_type_code" id="announcement_type_code" value='${rmHelper.returnParams.announcement_type_code}' displayLable="类型"></webTag:DynamicSelectTag> --%>
					...
					<div class="row" style="text-align:right;">
					    ...
					    <a class="btn btn-danger" href="<%=basePath%>/redirect/redirect.do?linkUrl=ca/cacore/ams/announcement/announcement_Add"><i class="icon-plus icon-white"></i>新增</a>
					</div><!-- /.row -->
				</form>
			</div>
			<!-- 查询面板 end -->
				...
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tbody>
						<c:forEach var="rb" items="${rmHelper.returnMsg.dataList}" varStatus="index" >
							<tr>
							<td width="25">
							          <%--   <c:if test="${rb.get('approval_status') == '0'||rb.get('approval_status') == '1'}"> --%>
									      <a class="btn btn-mini btn-dangerLight" href='<%=basePath %>/AMS/Announcement/viewAnnouncement.do?seq_id=${rb.seq_id}&pageName=announcement_Modify'><i class="icon-pencil icon-white"></i>修改</a>
							<%-- 		   </c:if>  --%>
