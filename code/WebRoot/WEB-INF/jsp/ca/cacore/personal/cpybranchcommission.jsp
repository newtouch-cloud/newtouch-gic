...
<html lang="en">
	<head>
		...
		<!--重置-->
			...
			var linkUrl = "<%=basePath %>/personal/cpybranchcommission.do";
			...
			function exportSalesInfo(){
			    $('#queryForm').attr("action","<%=basePath %>/personal/downloadcpybranchcommission.do"); //设置action指向导出
			    $('#queryForm').submit();
			    $('#queryForm').attr("action","<%=basePath %>/personal/cpybranchcommission.do"); //设置action执向查询
			 }
		</script>
	...
	<body>
		...
		<div class="row-fluid" id="Shrinkcontent1">
			<form id="queryForm" name="queryForm" class="form-horizontal alert alert-info fade in span12" action="<%=basePath%>/personal/cpybranchcommission.do" method="POST" autocomplete="off">
				 <!-- 提示信息 -->
