...
function cpygetOrgTree(){
	$.ajax({
		type : "POST",
		url :  "<%=basePath %>/sms/queryCpyBranchTree.do?rqstType=AJAX",
		async : false,
...
function zTreeOnExpand(event, treeId, treeNode) {
    ...
    $.ajax({
        type: "POST",
        url: "<%=basePath %>/sms/queryCpyBranchTree.do?rqstType=AJAX&org_level=" + org_level + "&org_id=" + org_id,
        async: false,
        success: function (data) {
            var zTree = $.fn.zTree.getZTreeObj("treeDemo");
            var zNodes = JSON.parse(data).retList;
...
<div style="margin-left:0px"  class='control-group span4'>
	<label   class='control-label' for='ins_branchname'>保险公司机构</label>
	<div class='controls' >
