...
%>
		<script type="text/javascript" src="<%=basePathjs %>/compent/jquery/jquery-1.9.1.min.js"></script>
		<!-- jQuery -->
	...
	<body>
		<div class="login">
			...
			<webTag:HiddenInputTag id="msg" name="msg" value="${msg}"/>
           ...
           <%-- <div class="div_input_text">用户名</div>--%>
            ...
            <div class="div_input div_inputPass">
                <input type="password" id="opt_password" name="opt_password" placeholder="请输入密码" class="pass" value="${opt_password}"/> 
            </div>
             <%--<div class="div_input_text">验证码</div>--%>
           <div class="div_input div_inputValidateCode">
                    <input type="text" id="validateCode" name="validateCode" placeholder="请输入验证码" class="code" value="${validateCode}"/>
					<img src="<%=basePath %>/sendValidateCode.controller?a=<%=r.nextDouble()%>" class="yanzhengma"  alt="验证码" title="点击更换" onclick="change(this);" />
	    	</div>
            	<input type="hidden" id="linkUrl" name="linkUrl" value="newtouch/login/indexBef"/>
            	<a  href="#"><button class="btns">登录</button></a>
				<div class="message" >${message}</div>    
 			<div class="messageNew" id="messageNew" >用户密码已过期，请<a href="#"  id="mdfpass" onclick="toUpdatePass()" >修改密码</a></div> 
            </form>	
        </div>
	</body>
</html>